...
- (void)viewDidLoad {
    ...
    NSLog(@"hhhhhhnnnnn%@",self.navigationController.navigationBar);
    UIView *navagationView  = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, 64)];
    navagationView.backgroundColor = [UIColor whiteColor];

    [self.view addSubview:navagationView];
    
    
     UIButton *leftbutton=[[UIButton alloc]initWithFrame:CGRectMake(0, 25, 80, 20)];
    ...
    for (QAVEndpoint *endpoint in endpoints) {
        switch (event) {
            case QAV_EVENT_ID_ENDPOINT_HAS_CAMERA_VIDEO:
            {
                ...
                NSLog(@"%f==%f==",renderView.frame.origin.x,renderView.frame.origin.y);
                renderView.frame = CGRectMake(0, 64, CGRectGetWidth(self.view.frame), CGRectGetHeight(self.view.frame)-64);
